...
public class ShadowForce : Attack
{
    public ShadowForce()
    {
        // #Definitions
        ...
        Name = Localization.GetString($"move_name_{ID}", "Shadow Force");
    ...
    {
        int shadowforce = battleScreen.FieldEffects.ShadowForceCounter.Self;
        if (own == false)
        {
    ...
    {
        int shadowforceCounter = battleScreen.FieldEffects.ShadowForceCounter.Self;

        if (own == false)
        {
            shadowforceCounter = battleScreen.FieldEffects.ShadowForceCounter.Opponent;
        }
        ...
        if (p.Item != null)
        {
            ...
            {
                if (battleScreen.Battle.RemoveHeldItem(own, own, battleScreen, "Power Herb pushed the use of Shadow Force!", "move:shadowforce") == true)
                {
                    hasToCharge = false;
                }
            }
        ...
        if (shadowforceCounter == 0 && hasToCharge == true)
        {
            if (own == true)
            {
                battleScreen.FieldEffects.ShadowForceCounter.Self = 1;
            }
            else
            {
                battleScreen.FieldEffects.ShadowForceCounter.Opponent = 1;
            }
