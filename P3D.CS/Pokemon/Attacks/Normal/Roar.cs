...
public class Roar : Attack
{
    public Roar()
    {
        ...
        Name = Localization.GetString($"move_name_{ID}", "Roar");
    ...
    public override void MoveHits(bool own, BattleScreen battleScreen)
    {
        ...
        if (BattleCalculation.CanSwitch(battleScreen, own == false) == true && p.Ability.Name.ToLower() != "suction cups")
        {
            ...
            if (battleScreen.IsPVPBattle == true || battleScreen.IsTrainerBattle == true || battleScreen.IsRemoteBattle == true)
            {
                ...
                else
                {
                    if (Core.Player.CountFightablePokemon > 1)
                    {
                        ...
                        while (Core.Player.Pokemons[i].IsEgg == true || Core.Player.Pokemons[i].Status == Pokemon.StatusProblems.Fainted || battleScreen.SelfPokemonIndex == i || Core.Player.Pokemons[i].HP <= 0)
                        {
