...
[Item(7, "Lava Cookie")]
public class LavaCookie : MedicineItem
{
    public override String Description { get; protected set; } = "Lavaridge Town's local specialty. It can be used once to heal all the status conditions of a Pokemon.";
    ...
    public override bool UseOnPokemon(int PokeIndex)
    {
        ...
        {
            if (Pokemon.Status != P3D.Pokemon.StatusProblems.None || Pokemon.HasVolatileStatus(Pokemon.VolatileStatus.Confusion) == true)
            {
                Pokemon.Status = P3D.Pokemon.StatusProblems.None;

                if (Pokemon.HasVolatileStatus(Pokemon.VolatileStatus.Confusion) == true)
                {
                ...
                String t = Pokemon.GetDisplayName() + "~eats the cookie...*" + Pokemon.GetDisplayName() + "~gets healed up!";
                t += RemoveItem();
