namespace Screens.MainMenu;

// Static integration helpers — full NewNewGameScreen class is in NewNewGameScreen.cs
public static class NewNewGameScreen
{
    public static void EndNewGame(String name, float arg1, float arg2, float arg3, int arg4) { }

    public static class CharacterSelectionScreen
    {
        public static String SelectedSkin = String.Empty;
        public static String SelectedName = String.Empty;
        public static String SelectedGender = String.Empty;
    }
}
